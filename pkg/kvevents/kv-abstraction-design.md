...
func (s *Subscriber) Start(ctx context.Context) error {
    ...
    // Get transport from adapter
    transport := s.adapter.GetTransport()
    
    // Subscriber decides how to connect
    if s.remote {
        // Per-pod: Connect to remote endpoint
        transport.Connect(ctx, s.endpoint)
    } else {
        // Global: Bind to local endpoint
        transport.Bind(ctx, s.endpoint)  // ← Need Bind() in Transport interface
    }
    
    // Receive loop
    go func() {
        for {
            rawBytes, _ := transport.Receive(ctx)
            batch, _ := s.adapter.ProcessMessage(ctx, rawBytes)
            s.pool.AddTask(&Message{...})
        }
    }()
